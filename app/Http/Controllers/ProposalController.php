...
class ProposalController extends Controller
{
    ...
    public function index(Request $request)
    {
        if (Auth::user()->can('proposal manage')) {
            ...
            } else if (Auth::user()->managedUsers()->exists()) {
                $users = User::where("workspace_id", getActiveWorkSpace())->where("reporter", Auth::user()->id)->get();
                $query = Proposal::where('workspace', getActiveWorkSpace())->whereIn('created_by_user', $users->pluck("id"));
            } else {
                $query = Proposal::where('workspace', getActiveWorkSpace())->where('created_by_user', Auth::user()->id);
            }
            ...
            $proposals = $query->get();
            return view('proposal.index', compact('proposals', 'customer', 'status', 'leads'));
        } else {
     ...
     * @return Renderable
     */
    public function create($customerId, $leadId = null)
    {
        if (module_is_active('ProductService')) {
            if (Auth::user()->can('proposal create')) {
                ...
                return view('proposal.create', compact('customers', 'proposal_number', 'product_services', 'category', 'customerId', 'projects', 'taxs', 'customFields', 'leadId'));
            } else {
                return redirect()->back()->with('error', __('Permission Denied.'));
            }
        } else {
    ...
    public function store(Request $request)
    {
        if (\Auth::user()->can('proposal create')) {
            if ($request->proposal_type == "product") {
                ...
                $proposal->created_by_user = Auth::user()->id;
            ...
            if ($proposal->workspace == getActiveWorkSpace()) {
                ...
                if (module_is_active('Account')) {
                    if ($proposal->type == "Lead") {
                        $customer = Lead::where('user_id', $proposal->lead->id)->where('workspace_id', getActiveWorkSpace())->first();
                    } else {
    ...
    {
        $latest = Proposal::where("workspace", "=", getActiveWorkSpace())->orderBy('id', 'desc')->first(); 
            if ($latest == null) {
            return 1;
        } else {
    ...
    public function customer(Request $request)
    {
        if (module_is_active('Account')) {
            ...
            if (empty($customer)) {
                ...
                if (empty($user)) {
                    $leads = Lead::where('id', $request->id)->where('workspace_id', getActiveWorkSpace())->where('created_by', creatorId())->first();
                    $customer['name'] = !empty($leads->company_name) ? $leads->company_name : (!empty($leads->name) ? $leads->name : '');
                    $customer['email'] = !empty($leads->email) ? $leads->email : '';
                } else {
    ...
    public function product(Request $request)
    {
        ...
        // new implementation
        $data['taxes'] = Tax::where('workspace_id', '=', getActiveWorkSpace())->get();
    ...
    public function resent($id)
    {
        if (Auth::user()->can('proposal send')) {
            ...
            if (!empty(company_setting('Proposal Status Updated')) && company_setting('Proposal Status Updated') == true) {
                ...
                try {
                    $resp = EmailTemplate::sendEmailTemplate('Proposal Send', [$customer->id => $customer->email], $uArr, null, null, Auth::user()->email);
                } catch (\Exception $e) {
    ...
    public function proposal($proposal_id)
    {
        ...
        if ($proposal->type == "Lead") {
            $lead = Lead::where("id", $proposal->customer_id)->first();
            $employee = Employee::where('user_id', $lead->created_by_user)->first();
        }
        if (module_is_active('Account')) {
            $customer = \Modules\Account\Entities\Customer::where('user_id', $proposal->customer_id)->first();
            if ($proposal->type == "Lead") {
                $customer = new \Modules\Account\Entities\Customer();
                $customer->billing_name = $lead->company_name;
                $customer->shipping_name = $lead->company_name;
                $customer->billing_address = $lead->company_address;
                $customer->shipping_address = $lead->company_address;
                $customer->billing_phone = $lead->phone;
                $customer->shipping_phone = $lead->phone;
            }
        } else {
            $customer = User::where('id', $proposal->customer_id)->first();
        }
            ...
            if (module_is_active('ProductService')) {
                ...
                if (!empty($item->tax)) {
                    ...
                    foreach (explode(',', $item->tax) as $tax) {
                        if (count(explode("_", $tax)) > 1) {
                            $taxPrice = Invoice::taxRate(explode("_", $tax)[1], $item->price, $item->quantity, $item->discount);
                            $tax_price += $taxPrice;
                            $totalTaxPrice += $taxPrice;

                            $itemTax['name'] = explode("_", $tax)[0];
                            $itemTax['rate'] = explode("_", $tax)[1] . '%';
                            $itemTax['price'] = currency_format_with_sym($taxPrice, $proposal->created_by);
                            $itemTaxes[] = $itemTax;


                            if (array_key_exists(explode("_", $tax)[0], $taxesData)) {
                                $taxesData[explode("_", $tax)[0]] = $taxesData[explode("_", $tax)[0]] + $taxPrice;
                            } else {
                                $taxesData[explode("_", $tax)[0]] = $taxPrice;
                            }
                        }
                    }
                    $item->itemTax = $itemTaxes;
        ...
        if ($proposal) {
            ...
            $settings['employee_name'] = $employee->name;
            $settings['employee_code'] = Employee::employeeIdFormat($employee->employee_id);
            $settings['employee_email'] = $employee->email;
            $settings['employee_phone'] = $employee->phone;
    ...
    public function previewInvoice($template, $color)
    {
        ...
        $settings['employee_name'] = '<Employee Name>';
        $settings['employee_code'] = '<Employee Code>';
        $settings['employee_email'] = '<Employee Email>';
        $settings['employee_phone'] = '<Employee Phone>';
    ...
    public function ProposalSectionGet(Request $request)
    {
        ...
        if ($request->type == "product" && module_is_active('Account')) {
            ...
            $returnHTML = view('proposal.section', compact('product_services', 'type', 'acction', 'proposal', 'product_services_count'))->render();
            $response = [
                'is_success' => true,
                'message' => '',
                'html' => $returnHTML,
