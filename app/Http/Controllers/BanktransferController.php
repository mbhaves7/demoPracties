    ...
    public function update(Request $request, $id)
    {
        ...
        if($bank_transfer_payment && $bank_transfer_payment->status == 'Pending')
        {
            ...
            if($request->status == 'Approved')
            {
                ...
                $user_counter = (isset($requests->user_counter_input)) ? $requests->user_counter_input : 0;
                ...
                $assignPlan= DirectAssignPlan($plan->id,$duration,$user_module,$user_counter,'Bank Transfer',$bank_transfer_payment->user_id);

                $user = User::find($bank_transfer_payment->user_id);
                $assignPlan = $user->assignPlan($plan->id,$duration,$user_module,$user_counter,$bank_transfer_payment->user_id);
