            ...
            <ul class="dash-navbar">
                <?php $__currentLoopData = getSideMenu(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $menu): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check($menu->permissions)): ?>
                        ...
                        <?php if($staus == true &&
    (
        !($menu->title == "Job Description" && !(strcasecmp(Auth()->user()->type, "company") == 0 || strcasecmp(Auth()->user()->type, "hr") == 0 || Auth::user()->managedUsers()->exists()))
        || $menu->title != "Job Description"
    )): ?>
...
<?php /**PATH D:\QuantmHill\erp_system\resources\views/partials/sidebar.blade.php ENDPATH**/ ?>